  ...
  // ─── 3.9.x  Admin DB stats, sync hardening  (Apr 21–23, 2026) ──────────────
  {
    ...
    changes: [
      { cat: 'Feature', items: [
        ...
        'Database capacity guard (DB_CAPACITY_BYTES) added to all scrape endpoints and all three Python scrapers — scraping pauses automatically at 95% usage (configurable via DB_PAUSE_THRESHOLD_PERCENT)',
        'GitHub Actions workflows (scrape.yml, scrape-retail-menus.yml) now pass DB_CAPACITY_BYTES, DB_PAUSE_THRESHOLD_PERCENT, and DB_CAPACITY_STRICT to scrapers',
        ...
        'Sync: auto-prune stale sync rows older than SYNC_RETENTION_DAYS (default 30) on every request (hourly throttle); updated_at index added for performance',
        'Scrape status endpoint now includes db_guard capacity info in every response',
      ...
      { cat: 'Technical', items: [
        'Tests: added scrape-guard.test.js verifying both scrape routes return 503 when capacity guard triggers',
        'Tests: sync-api.test.js updated to cover sync data pruning on POST',
      ]},
    ],
  },
...

  {
    ...
    changes: [
      { cat: 'UI', items: [
        ...
        'Compare page: improved "No items" empty state — distinguishes between no search matches and no menu data, with date hint',
        'Compare page: added ⇄ Swap button to instantly exchange Court A and Court B selections',
        'Custom Foods page: added Sign in with Google button to auth gate — users no longer hit a dead end',
        'Custom Foods page: loading state now shows skeleton rows instead of plain text',
        'Profile page: added "Calculate via TDEE calculator →" link below Edit Goals for discoverability',
        'About page: added Tools link to Navigate sidebar; reordered links',
        'Privacy page: added Compare and Tools to Navigate sidebar; updated last-modified date to March 2026',
      ]},
      { cat: 'SEO', items: [
        ...
        'Tools page: updated title to "TDEE & BMI Calculator — BoilerFuel" for search clarity',
      ]},
    ],
  },
