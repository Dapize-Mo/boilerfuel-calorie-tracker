  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      if (meal_time) {
        ...
        // Beverages are available at all meal periods — skip meal_time filter for them
        conditions.push(`(LOWER(station) = 'beverages' OR LOWER(meal_time) = $${p} OR LOWER(meal_time) LIKE $${p} || '/%' OR LOWER(meal_time) LIKE '%/' || $${p} OR LOWER(meal_time) LIKE '%/' || $${p} || '/%')`);
      }
      if (station) {
