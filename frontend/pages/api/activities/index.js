...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      ...
      const { rows } = await query(`SELECT id, name, calories_per_hour FROM activities ${where} ORDER BY name ASC`, params);
  ...
  if (req.method === 'POST') {
    try {
      ...
      await ensureSchema();
      const body = req.body || {};
      const { name, calories_per_hour } = body;
      if (!name || calories_per_hour == null) {
        return res.status(400).json({ error: 'Missing required fields' });
      }
      const result = await query(
        `INSERT INTO activities (name, calories_per_hour) VALUES ($1,$2) RETURNING id, name, calories_per_hour`,
        [String(name).trim(), Number(calories_per_hour)]
      );
